    ...
    public class Contact
    {
        ...
        public bool Supprimer()
        {
            try
            {
                ...
                if (dbCon.IsConnect())
                {
                    String sqlString = "DELETE FROM client WHERE id = ?id";
        ...
        private void UpdateParticipant()
        {
            try
            {
                ...
                if (dbCon.IsConnect())
                {
                    String sqlString = "UPDATE client SET nom = ?nom,prenom = ?prenom,email = ?email WHERE Id = ?id";
                    sqlString = Tools.PrepareLigne(sqlString, "?id", Tools.PrepareChamp(Id.ToString(), "Nombre"));
        ...
        private void AddParticipant()
        {
            try
            {
                ...
                if (dbCon.IsConnect())
                {
                    String sqlString = "INSERT INTO client(nom,prenom,email) VALUES(?nom,?prenom,?email)";
                    sqlString = Tools.PrepareLigne(sqlString, "?id", Tools.PrepareChamp(Id.ToString(), "Nombre"));
                    sqlString = Tools.PrepareLigne(sqlString, "?nom", Tools.PrepareChamp(Nom, "Chaine"));
                    sqlString = Tools.PrepareLigne(sqlString, "?prenom", Tools.PrepareChamp(Prenom, "Chaine"));
                    sqlString = Tools.PrepareLigne(sqlString, "?email", Tools.PrepareChamp(Email, "Chaine"));
